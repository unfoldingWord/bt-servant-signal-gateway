...
rate limit signal-cli/Signal-Server enforce. The attachment send path calls
``acquire(n)`` before an attachment send; on a 429 it calls
``feedback(retry_after, n)`` so the model recalibrates from the server's
authoritative hint.

The scheduler serializes concurrent calls through an ``asyncio.Lock``, giving
FIFO fairness across sends sharing one signal-cli daemon.

Ported from ``../hermes-agent/gateway/platforms/signal_rate_limit.py``.
"""
...
class SignalRateLimitError(Exception):
    """Raised by ``SignalClient._rpc`` for rate-limit responses when the caller
...
# RateLimitException, so the typed path doesn't fire).
...
def _is_signal_rate_limit_error(err: Any) -> bool:
    ...
    - libsignal-net's ``RetryLaterException`` / ``Retry after N seconds``
      surfaced inside ``AttachmentInvalidException`` when the rate limit is hit
      during attachment upload — signal-cli never re-tags these as
...
def _signal_send_timeout(num_attachments: int) -> float:
    ...
    time scales with batch size. Default 30s is fine for text-only sends but
    truncates large attachment batches mid-upload — we then log a phantom
    failure even though signal-cli completes the send a few seconds later.
...
class SignalAttachmentScheduler:
    ...
    per-token Retry-After hint we get from the server when a 429 fires. Until
    we've observed one, we use the documented default (1 token / 4 seconds).

    Concurrent ``acquire(n)`` calls serialize through an ``asyncio.Lock`` —
    natural FIFO across sends hitting the same daemon.
    """
    ...
    def estimate_wait(self, n: int) -> float:
        ...
        *before* committing to an ``acquire`` that may block silently.
        Lock-free; small races vs. concurrent acquires are benign for an
    ...
    def state(self) -> dict:
        ...
        return {
            ...
            "refill_seconds_per_token": (
                round(1.0 / self.refill_rate, 1) if self.refill_rate > 0 else float("inf")
            ),
        }
...
def _reset_scheduler() -> None:
    """Drop the cached scheduler so the next ``get_scheduler`` call builds a
    fresh one. Test-only — never call from production paths."""
    global _scheduler
    _scheduler = None
